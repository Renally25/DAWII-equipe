...
import {
  createTelefoneSchema,
  deleteTelefoneSchema
} from "@/lib/validators";
...
export async function POST(requisicao) {
  try {
    ...
    const parsed = createTelefoneSchema.safeParse(body);
    ...
    const { CodUsuario, telefone } = parsed.data;

    await pool.query(
      `INSERT INTO Telefone (CodUsuario, telefone, ativo)
       VALUES ($1, $2, true)`,
      [CodUsuario, telefone]
    );
...
export async function DELETE(requisicao) {
  try {
    ...
    const parsed = deleteTelefoneSchema.safeParse({
    ...
    return NextResponse.json({
      message: "Telefone inativado com sucesso!"
    });
